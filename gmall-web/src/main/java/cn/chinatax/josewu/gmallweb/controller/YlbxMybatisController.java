...
@Controller
@RequestMapping("ylbx")
public class YlbxMybatisController extends EasyExcelBaseController{

    private final  static  String ylbxKey  = "qyyl";
    @Autowired
    private RedisServiceImpl redisServece;

    @Autowired
    private ZsxxMapper ylbxMapper;


    private  static  final Logger log = LoggerFactory.getLogger(YlbxMybatisController.class);


    @GetMapping("insertYlbx")
    @ResponseBody
    public Object insertAll(HttpServletResponse response){

        try{
            String  keyString = redisServece.getString(ylbxKey);
            String  jsonString = redisServece.getString(keyString);
            System.out.println("%%%%%%=>"+jsonString);
            List<Zsxx> zsxxLists = JSON.parseObject(jsonString,new TypeReference<List<Zsxx>>(){});
            for(Zsxx zsxx : zsxxLists){
                    long  longId = Long.parseLong(UUIDNumber.getGuid());
                    zsxx.setId(longId);
                    //log.info(zsxx.getDq());
                    ylbxMapper.insert(zsxx);
            }
            return success("导出成功");
        }catch(Exception e) {
            log.info(e.toString());
